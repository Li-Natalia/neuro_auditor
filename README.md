...
**Платформа аудита финансовой отчётности с AI-ассистентом:** загрузка Excel-отчётов (*МСФО/РСБУ*), автоматический расчёт коэффициентов и выявление рисков, дашборд и PDF, а также чат-бот по документу на базе *YandexGPT* (`Yandex AI Studio`, Responses API) с режимом Code Interpreter для анализа файла на стороне AI Studio.
